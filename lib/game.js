...
export function db() {
  ...
  if (!url || !token) throw new Error("Database not connected: KV_REST_API_URL / KV_REST_API_TOKEN are missing. Connect Upstash Redis in Vercel → Storage, then Redeploy.");
...
export const FOODS = {
  snack: { id: "snack", name: "Snack",     cooldown: 1 * 3600e3,  min: 20,  max: 60,   cost: 10 },
  meal:  { id: "meal",  name: "Meal",      cooldown: 6 * 3600e3,  min: 150, max: 400,  cost: 40,  nft: true },
  big:   { id: "big",   name: "Big Order", cooldown: 12 * 3600e3, min: 500, max: 1200, cost: 100, nft: true },
  choc:  { id: "choc",  name: "Chocolate", cooldown: 12 * 3600e3, min: 800, max: 2000, buy: 150 },
};
...
export async function savePlayer(p) {
  await db().set(pkey(p.id), p);
  await db().zadd(lbkey(p.week), { score: p.calories, member: p.name });
}

export async function createPlayer(id, name) {
  name = String(name || "").trim().slice(0, 10);
  if (!name) throw new Error("Name required");
  const taken = await db().get(namekey(name));
...
export async function leaderboard(p) {
  const week = weekId();
  const raw = await db().zrange(lbkey(week), 0, -1, { rev: true, withScores: true });
